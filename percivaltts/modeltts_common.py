...
class DCNNF0SpecNoiseFeatures(modeltts.ModelTTS):
    def __init__(self, ctxsize, vocoder, cfgarch, nameprefix=None):
        ...
        # F0
        l_f0 = l_ctx
        l_f0 = kl.Conv1D(cfgarch.arch_gen_nbfilters, int(0.5*0.200/vocoder.shift)*2+1, strides=1, padding='same', use_bias=False)(l_f0)
        ...
        for _ in xrange(cfgarch.arch_gen_nbcnnlayers):
            l_spec = kl.Conv2D(cfgarch.arch_gen_nbfilters, [cfgarch.arch_winlen,cfgarch.arch_spec_freqlen], strides=(1, 1), padding='same', dilation_rate=(1, 1), use_bias=False, data_format='channels_last')(l_spec)
            l_spec = kl.BatchNormalization(axis=bn_axis)(l_spec)
            l_spec = kl.LeakyReLU(alpha=0.3)(l_spec)
        ...
        if 0:
            ...
            for _ in xrange(cfgarch.arch_gen_nbcnnlayers/2):
                l_nm = kl.Conv2D(cfgarch.arch_gen_nbfilters, [cfgarch.arch_winlen,cfgarch.arch_noise_freqlen], strides=(1, 1), padding='same', dilation_rate=(1, 1), use_bias=False, data_format='channels_last')(l_nm)
                l_nm = kl.BatchNormalization(axis=bn_axis)(l_nm)
                l_nm = kl.LeakyReLU(alpha=0.3)(l_nm)
            ...
            l_nm = l_ctx
            for _ in xrange(cfgarch.arch_gen_nbcnnlayers/2):
                l_nm = kl.Dense(cfgarch.arch_hiddenwidth, use_bias=False)(l_nm)   # Projection
                l_nm = kl.BatchNormalization(axis=bn_axis)(l_nm)
                l_nm = kl.LeakyReLU(alpha=0.3)(l_nm)
            l_nm = kl.Dense(vocoder.noisesize(), activation='sigmoid')(l_nm)  # TODO Sigmoid is for PML !
